...
const HomePage = () => {
  ...
  return (
    <div className="min-h-screen bg-muted-50 w-full">
      ...
      {/* Hero Section */}
      <section className="bg-gradient-to-r from-primary-700/8 to-indigo-700/6 py-12 sm:py-16 md:py-20 w-full">
          ...
          >
            {recentReviews.map((review, index) => (
              <motion.div
                ...
                transition={{ duration: 0.2, ease: "easeInOut" }}
                className="bg-card-100 p-4 sm:p-6 rounded-lg shadow-md transition-all duration-200 ease-in-out hover:shadow-lg"
              >
                <div className="flex items-center mb-3 sm:mb-4">
                  <div className="flex text-amber-400">
                    {[...Array(5)].map((_, i) => (
                      <svg key={i} xmlns="http://www.w3.org/2000/svg" className={`h-4 w-4 sm:h-5 sm:w-5 ${i < review.rating ? 'text-amber-400' : 'text-gray-300'}`} viewBox="0 0 20 20" fill="currentColor">
                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                      </svg>
                    ))}
                  </div>
                  <span className="ml-2 text-muted-500 text-sm sm:text-base">({review.rating})</span>
                </div>
                <p className="text-text-900 mb-3 sm:mb-4 leading-relaxed text-sm sm:text-base">"{review.comment}"</p>
                <div className="flex justify-between items-center">
                  <div>
                    <p className="font-semibold text-text-900 text-sm sm:text-base">{review.userName}</p>
                    <p className="text-xs sm:text-sm text-muted-500">{review.courseTitle}</p>
                  </div>
                  <div className="flex items-center text-muted-500">
                    ...
                    <span className="text-sm sm:text-base">{review.upvotes}</span>
                  </div>
                </div>
              </motion.div>
            ))}
      ...
      {/* Newsletter */}
      <motion.section 
        className="py-12 sm:py-14 md:py-16 bg-primary-700 text-text-900 w-full"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        ...
        <div className="container mx-auto px-4 text-center">
          ...
          >
            <input 
              ...
              className="flex-grow px-4 py-3 rounded-lg sm:rounded-l-lg sm:rounded-r-none text-text-900 focus:outline-none focus:ring-2 focus:ring-accent-500 bg-white/95 transition-all duration-200 ease-in-out text-sm sm:text-base"
            />
            <motion.button 
              className="bg-accent-500 text-text-900 font-semibold px-4 sm:px-6 py-3 rounded-lg sm:rounded-r-lg sm:rounded-l-none hover:bg-opacity-90 transition-all duration-200 ease-in-out shadow-md hover:shadow-lg text-sm sm:text-base"
      ...
      {/* Footer */}
      <motion.footer 
        className="bg-text-900 text-card-100 py-8 sm:py-10 md:py-12 w-full"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ duration: 0.7, delay: 0.3 }}
      >
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 sm:gap-8">
            <motion.div variants={item}>
              <h3 className="text-xl sm:text-h3 font-extrabold mb-3 sm:mb-4">CourseFinder</h3>
              <p className="text-muted-500 leading-relaxed text-sm sm:text-base">Find the best online courses from 50+ platforms in one place.</p>
            </motion.div>
            <motion.div variants={item}>
              <h4 className="font-semibold mb-3 sm:mb-4 text-base sm:text-lg">Products</h4>
              <ul className="space-y-2 text-muted-500">
                <li><a href="#" className="hover:text-card-100 transition-colors duration-200 ease-in-out text-sm sm:text-base">Course Comparison</a></li>
                <li><a href="#" className="hover:text-card-100 transition-colors duration-200 ease-in-out text-sm sm:text-base">Reviews</a></li>
                <li><a href="#" className="hover:text-card-100 transition-colors duration-200 ease-in-out text-sm sm:text-base">Learning Paths</a></li>
                <li><a href="#" className="hover:text-card-100 transition-colors duration-200 ease-in-out text-sm sm:text-base">Career Guides</a></li>
              </ul>
            </motion.div>
            <motion.div variants={item}>
              <h4 className="font-semibold mb-3 sm:mb-4 text-base sm:text-lg">Company</h4>
              <ul className="space-y-2 text-muted-500">
                <li><a href="#" className="hover:text-card-100 transition-colors duration-200 ease-in-out text-sm sm:text-base">About Us</a></li>
                <li><a href="#" className="hover:text-card-100 transition-colors duration-200 ease-in-out text-sm sm:text-base">Careers</a></li>
                <li><a href="#" className="hover:text-card-100 transition-colors duration-200 ease-in-out text-sm sm:text-base">Contact</a></li>
                <li><a href="#" className="hover:text-card-100 transition-colors duration-200 ease-in-out text-sm sm:text-base">Blog</a></li>
              </ul>
            </motion.div>
            <motion.div variants={item}>
              <h4 className="font-semibold mb-3 sm:mb-4 text-base sm:text-lg">Legal</h4>
              <ul className="space-y-2 text-muted-500">
                <li><a href="#" className="hover:text-card-100 transition-colors duration-200 ease-in-out text-sm sm:text-base">Privacy Policy</a></li>
                <li><a href="#" className="hover:text-card-100 transition-colors duration-200 ease-in-out text-sm sm:text-base">Terms of Service</a></li>
                <li><a href="#" className="hover:text-card-100 transition-colors duration-200 ease-in-out text-sm sm:text-base">Cookie Policy</a></li>
              </ul>
            </motion.div>
          </div>
          <motion.div 
            className="border-t border-muted-500 mt-6 sm:mt-8 pt-6 sm:pt-8 flex flex-col md:flex-row justify-between items-center gap-4"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ duration: 0.5, delay: 0.5 }}
          >
            <p className="text-muted-500 text-sm sm:text-base">© 2025 CourseFinder. All rights reserved.</p>
